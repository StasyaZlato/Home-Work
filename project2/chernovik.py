...
def barchart_create():
    output_file(os.path.join(".\\templates", "bars1.html"))
    ...
    with open(os.path.join('.\\templates', 'bars1.html'), 'w', encoding='utf-8') as f:
        f.write(html)
    return p
...
def index():
    ...
    temp = '''<body>
        ...
        <hr/>'''
    with open(os.path.join('.\\templates', 'bars1.html'), 'r', encoding='utf-8') as f:
        bars_html = f.read()
    if re.search(temp, bars_html) == None:
        bars_html = bars_html.replace('<body>', temp)
    with open(os.path.join('.\\templates', 'bars1.html'), 'w', encoding='utf-8') as k:
        k.write(bars_html)

    # show(p)
    # script, div = components(p)
    # return render_template("bars1.html", the_div=div, the_script=script)
    return render_template("bars1.html")
